...
const Toolbar: React.FC<ToolbarProps> = ({ selectionMode, setSelectionMode }) => {
    ...
    return (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2 bg-white/90 dark:bg-slate-800/90 backdrop-blur-md px-3 py-2 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 transition-all">
            <div className="relative">
                ...
                <input
                    ...
                    className="pl-8 pr-3 py-1.5 w-40 text-xs bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500 transition-all"
                />
                {searchQuery && (
                    <div className="absolute top-full left-0 mt-2 w-64 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                        {nodes.filter(n => (n.data.title || '').toLowerCase().includes(searchQuery.toLowerCase())).map(n => (
                            <button key={n.id} onClick={() => focusNode(n)} className="w-full text-left px-4 py-2 hover:bg-sky-50 dark:hover:bg-slate-700 text-xs font-semibold">{n.data.title || n.id}</button>
                        ))}
                    </div>
                )}
            </div>
...
const FlowCanvas = () => {
    ...
    return (
        <div className="flex-1 h-full w-full relative bg-slate-50 dark:bg-slate-950" ref={reactFlowWrapper} onDragEnter={onDragEnter}>
            ...
            >
                ...
                <Controls
                    ...
                    className="!bg-white dark:!bg-slate-800 !border-2 !border-slate-300 dark:!border-slate-600 shadow-xl rounded-lg !mb-8 !z-20 overflow-hidden"
